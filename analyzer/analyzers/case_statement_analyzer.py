...
class CaseStatementAnalyzer(BaseAnalyzer):
    ...
    def _check_case_coalesce_alternative(
        self, sql_text: str, context: AnalysisContext
    ) -> None:
    ...
    def _check_case_in_select(self, sql_text: str, context: AnalysisContext) -> None:
        ...
        if "SELECT" in sql_text and "CASE" in sql_text:
            ...
            from_start = (
                sql_text.find("FROM", select_start)
                if "FROM" in sql_text
                else len(sql_text)
            )
    ...
    def _check_searched_vs_simple_case(
        self, sql_text: str, context: AnalysisContext
    ) -> None:
