...
class GroupByAnalyzer(BaseAnalyzer):
    ...
    def _check_groupby_indexing(self, sql_text: str, context: AnalysisContext) -> None:
        ...
        group_by_match = re.search(
            r"GROUP BY\s+(.+?)(?:\s+HAVING|\s+ORDER|\s+LIMIT|;|$)", sql_text
        )
        if group_by_match:
    ...
    def _check_groupby_expressions(
    ...
    def _check_groupby_column_count(
        self, sql_text: str, context: AnalysisContext
    ) -> None:
    ...
    def _check_aggregate_functions(
        ...
        aggregate_count = sum(
            sql_text.count(func) for func in ["COUNT(", "SUM(", "AVG(", "MAX(", "MIN("]
        )
