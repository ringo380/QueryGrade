...
def user_stats_api(request):
    ...
    if not user_history.exists():
        return Response({"total_queries": 0, "message": "No query history found"})
    ...
    # Average score
    avg_score = (
        user_history.aggregate(avg_score=Avg("query__analysis__score"))["avg_score"]
        or 0
    )
    ...
    stats = {
        ...
        "recent_activity": {"queries_last_30_days": recent_queries},
        "most_common_database": user_history.values("database_type")
        .annotate(count=Count("database_type"))
        .order_by("-count")
        .first(),
    }
