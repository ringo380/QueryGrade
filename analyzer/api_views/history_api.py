...
from ..serializers import QueryAnalysisSerializer, QueryHistoryListSerializer
...
class QueryHistoryListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            UserQueryHistory.objects.filter(user=self.request.user)
            .select_related("query", "query__analysis")
...
def delete_query_history(request):
    ...
    with transaction.atomic():
        deleted_count, _ = UserQueryHistory.objects.filter(
            id__in=query_ids, user=request.user
        ).delete()
