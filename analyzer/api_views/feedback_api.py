...
def submit_feedback_api(request, analysis_id):
    ...
    if existing_feedback:
        ...
        serializer = QueryFeedbackSerializer(
            existing_feedback, data=request.data, partial=True
        )
    else:
        # Create new feedback
    ...
    try:
        if existing_feedback:
            ...
            feedback = QueryFeedback.objects.create(
                user_history=user_history, **serializer.validated_data
            )
            action = "created"
        ...
        logger.error(
            f"Error saving feedback via API for user {request.user.username}: {e}"
        )
        return Response(
