...
class DatabaseIntrospector:
    ...
    def _create_django_db_config(self) -> Dict[str, Any]:
        ...
        config = {
            'ENGINE': engine_map.get(self.config['engine'], 'django.db.backends.postgresql'),
            ...
            with self.connection.cursor() as cursor:
                ...
                for table_info in table_names:
                    ...
                    try:
                        ...
                        logger.debug(f"Could not get size info for table {table_name}: {e}")
            ...
            for table_name in table_references:
                ...
                if table_info:
                    context['schema_analysis'][table_name] = {
                        ...
                        'has_primary_key': any(idx.get('primary', False) for idx in table_info.indexes)
                    }
            ...
            if token.ttype is None and hasattr(token, 'tokens'):
                ...
                if any(keyword in str(token).upper() for keyword in ['FROM', 'JOIN']):
                    ...
                    for i, word in enumerate(words):
                        if word.upper() in ['FROM', 'JOIN', 'INNER', 'LEFT', 'RIGHT', 'FULL', 'OUTER']:
                            if i + 1 < len(words):
                                table_name = words[i + 1].strip('(),')
                                if table_name and table_name.upper() not in ['SELECT', 'WHERE', 'ORDER', 'GROUP']:
    ...
    def _generate_context_recommendations(self, query: str, table_name: str,
                                        table_info: TableInfo, columns: List[str]) -> List[str]:
        ...
        for column in columns:
            if column not in indexed_columns and column in [col['name'] for col in table_info.columns]:
        ...
        if 'JOIN' in query.upper():
            fk_columns = [fk['columns'][0] for fk in table_info.foreign_keys if fk['columns']]
            if fk_columns:
                recommendations.append(
