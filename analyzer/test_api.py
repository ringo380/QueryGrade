...
class QueryGradingAPITestCase(TransactionTestCase):
    ...
    def test_grade_query_api_success(self):
        ...
        self.assertTrue(
            QueryAnalysis.objects.filter(id=response.data["analysis_id"]).exists()
        )
    ...
    def test_delete_query_history_success(self):
        ...
        # Create test queries
        grade_url = reverse("api:grade_query")
        query1_response = self.client.post(
            grade_url, {"sql_text": "SELECT * FROM test1"}, format="json"
        )
        query2_response = self.client.post(
            grade_url, {"sql_text": "SELECT * FROM test2"}, format="json"
        )
        ...
        delete_response = self.client.delete(
            delete_url, {"query_ids": history_ids}, format="json"
        )
    ...
    def test_delete_query_history_invalid_input(self):
        ...
        response = self.client.delete(
            delete_url, {"query_ids": "invalid"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Test with invalid integer values
        response = self.client.delete(
            delete_url, {"query_ids": ["abc", "def"]}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_delete_query_history_not_found(self):
        ...
        response = self.client.delete(
            delete_url, {"query_ids": [99999, 99998]}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertIn("error", response.data)
    ...
    def test_delete_query_history_access_control(self):
        ...
        grade_url = reverse("api:grade_query")
        self.client.post(
            grade_url, {"sql_text": "SELECT * FROM my_query"}, format="json"
        )
        ...
        delete_url = reverse("api:delete_query_history")
        response = self.client.delete(
            delete_url, {"query_ids": [history_id]}, format="json"
        )
