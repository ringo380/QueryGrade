...
from ..models import UserQueryHistory, QueryFeedback, FeedbackLearning
...
def feedback_analytics(request):
    ...
    statistics = {
        ...
        'avg_accuracy': feedback_data.aggregate(Avg('accuracy_rating'))['accuracy_rating__avg'] or 0,
        'avg_usefulness': feedback_data.aggregate(Avg('usefulness_rating'))['usefulness_rating__avg'] or 0,
        'avg_clarity': feedback_data.aggregate(Avg('clarity_rating'))['clarity_rating__avg'] or 0,
        'would_recommend_count': feedback_data.filter(would_recommend=True).count(),
        'rating_distribution': feedback_data.values('accuracy_rating').annotate(count=Count('id')),
    }

    context = {
        'statistics': statistics,
        'recent_feedback': feedback_data.select_related('user_history', 'user_history__user').order_by('-created_at')[:10],
    }
