...
def async_processing_status(request):
    ...
    task_id = request.session.get('log_processing_task_id') or \
              request.session.get('batch_analysis_task_id') or \
              request.session.get('report_task_id')
    ...
    try:
        ...
        if task.ready():
            if task.successful():
                ...
                session_keys = ['log_processing_task_id', 'batch_analysis_task_id', 'report_task_id']
