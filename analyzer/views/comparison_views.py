...
def compare_results(request):
    ...
    if not comparison_data:
        messages.error(
            request, "No comparison data found. Please submit queries for comparison."
        )
    ...
    try:
        ...
        if comparison_data.get("query_3") and comparison_data["query_3"].strip():
            queries_to_analyze.append(
                (comparison_data["query_3"], comparison_data["query_3_name"])
            )
        ...
        for query_sql, query_name in queries_to_analyze:
            try:
                query, analysis = analyze_query(
                    query_sql, comparison_data["database_type"]
                )
                ...
                if analysis.issues_found and len(analysis.issues_found) > 0:
                    try:
                        ...
                        logger.warning(
                            f"Failed to generate optimization suggestions: {e}"
                        )
...
def generate_comparison_summary(results):
    ...
    if summary["best_query"] and summary["worst_query"]:
        if summary["best_query"] == summary["worst_query"]:
            formatted_summary.append(
                f"All queries performed equally with grade {summary['best_grade']}."
            )
        else:
            formatted_summary.append(
                f"Best performing query: '{summary['best_query']}' (Grade {summary['best_grade']})"
            )
            formatted_summary.append(
                f"Query needing most improvement: '{summary['worst_query']}' (Grade {summary['worst_grade']})"
            )
    ...
    if summary["performance_ranking"]:
        ...
        for i, query in enumerate(summary["performance_ranking"], 1):
            formatted_summary.append(
                f"{i}. {query['name']} - Grade {query['grade']} ({query['score']:.1f} points)"
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.error(
                    request,
                    f"An error occurred while processing your queries: {str(e)}",
                )
...
def batch_results(request):
    ...
    if not batch_data:
        messages.error(
            request, "No batch analysis data found. Please submit queries for analysis."
        )
