        ...
        if form.is_valid():
            try:
                ...
                if introspector.connect():
                    ...
                    messages.success(
                        request,
                        f"Successfully connected to {db_config['engine']} database '{db_config['name']}'!",
                    )
                ...
                logger.error(
                    f"Database connection error for user {request.user.username}: {e}"
                )
...
def database_schema(request):
    ...
    if not db_config:
        messages.error(
            request, "No database connection found. Please connect to a database first."
        )
    ...
    try:
        ...
        if introspector.connect():
            ...
            messages.error(
                request,
                "Failed to reconnect to the database. Please check your connection.",
            )
            return redirect("database_analyze")
        ...
        if form.is_valid():
            try:
                ...
                if introspector.connect():
                    ...
                    messages.error(
                        request,
                        "Failed to reconnect to the database. Using standard analysis.",
                    )
                    return redirect("grade_query")

            except Exception as e:
                logger.error(
                    f"Contextualized query analysis error for user {request.user.username}: {e}"
                )
...
def contextualized_results(request, analysis_id):
    ...
    try:
        ...
        messages.error(
            request, "Analysis not found or you don't have permission to view it."
        )
        return redirect("query_history")
    except Exception as e:
        logger.error(
            f"Error displaying contextualized results for user {request.user.username}: {e}"
        )
...
def analyze_database_schema(tables, db_config):
    ...
    for table in tables:
        ...
        if table.row_count and table.row_count > 1000000:
            analysis["performance_notes"].append(
                f"Table '{table.name}' has {table.row_count:,} rows (consider partitioning)"
            )
        ...
        for fk in table.foreign_keys:
            if fk["columns"]:
                ...
                if fk_column not in indexed_columns:
                    analysis["issues"].append(
                        f"Foreign key column '{table.name}.{fk_column}' should have an index"
                    )
