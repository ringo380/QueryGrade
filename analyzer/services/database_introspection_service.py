...
class DatabaseIntrospectionService:
    ...
    def connect_to_database(
        ...
        try:
            ...
            if self.introspector.connect():
                logger.info(
                    f"Successfully connected to {config.engine} database '{config.name}'"
                )
                return True, None
            else:
                return (
    ...
    def analyze_schema(self, schema: Optional[str] = None) -> SchemaAnalysisResult:
        ...
        if not self.introspector:
            raise ValueError(
                "Not connected to a database. Call connect_to_database first."
            )
        ...
        try:
            ...
            for table_name in tables:
                ...
                table_recommendations = self._analyze_table(
                    table_name, columns, indexes, foreign_keys
                )

                all_recommendations.extend(table_recommendations)

                # Check for missing indexes
                missing = self._find_missing_indexes(
                    table_name, columns, indexes, foreign_keys
                )
                missing_indexes.extend(missing)

                # Check for foreign key issues
                fk_issues = self._analyze_foreign_keys(
                    table_name, foreign_keys, indexes
                )
    ...
    def _analyze_foreign_keys(
        self, table_name: str, foreign_keys: List[Dict], indexes: List[Dict]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_statistics(self, tables: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_indexes_per_table": (
                round(total_indexes / len(tables), 2) if tables else 0
            ),
        }
