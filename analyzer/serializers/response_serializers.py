...
class QueryGradeResponseSerializer(serializers.Serializer):
    ...
    recommendations = serializers.ListField(
        child=serializers.CharField(), help_text="List of improvement recommendations"
    )
    performance_notes = serializers.CharField(help_text="Performance analysis notes")
    execution_time_ms = serializers.IntegerField(
        help_text="Analysis execution time in milliseconds"
    )
    created_at = serializers.DateTimeField(
        help_text="Timestamp when analysis was created"
    )
...
class BatchQueryResponseSerializer(serializers.Serializer):
    ...
    total_queries = serializers.IntegerField(
        help_text="Total number of queries analyzed"
    )
    successful_analyses = serializers.IntegerField(
        help_text="Number of successful analyses"
    )
    failed_analyses = serializers.IntegerField(help_text="Number of failed analyses")
    average_grade = serializers.CharField(
        help_text="Average grade across all successful analyses"
    )
    average_score = serializers.FloatField(
        help_text="Average score across all successful analyses"
    )
    results = QueryGradeResponseSerializer(
        many=True, help_text="Individual analysis results"
    )
    summary = serializers.CharField(help_text="Summary of batch analysis results")
    created_at = serializers.DateTimeField(
        help_text="Timestamp when batch analysis was created"
    )
