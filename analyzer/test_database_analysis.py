    ...
    def test_analyze_query_context_mock(self):
        ...
        with patch.object(self.introspector, 'connection', Mock()):
            with patch('analyzer.database_introspector.sqlparse') as mock_sqlparse:
                ...
                with patch.object(self.introspector, '_extract_table_references', return_value=['users']):
                    with patch.object(self.introspector, '_extract_column_references', return_value=['active']):
                        with patch.object(self.introspector, '_tables_cache', {'users': self._create_mock_table()}):
    ...
    def test_query_with_context_post(self):
        ...
        with patch('analyzer.views.DatabaseIntrospector') as mock_introspector_class:
            with patch('analyzer.views.analyze_query') as mock_analyze_query:
                ...
                response = self.client.post(reverse('query_with_context'), data=form_data)
                self.assertRedirects(response, reverse('contextualized_results', args=[1]))
...
class DatabaseSchemaAnalysisTest(TestCase):
    ...
    def _create_good_table(self):
        ...
        table.indexes = [
            {'name': 'primary', 'columns': ['id'], 'primary': True, 'unique': True},
            {'name': 'idx_email', 'columns': ['email'], 'primary': False, 'unique': True}
        ]
        table.foreign_keys = []
        return table
    ...
    def _create_table_with_many_columns(self):
        ...
        table.columns = [
            {'name': f'col_{i}', 'type': 'varchar'} for i in range(25)
        ]
        table.indexes = [
            {'name': 'primary', 'columns': ['col_0'], 'primary': True}
        ]
        table.foreign_keys = []
        return table