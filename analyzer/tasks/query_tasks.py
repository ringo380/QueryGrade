...
def batch_analyze_queries(
    self, query_data_list: List[Dict], user_id: int
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Starting batch analysis of {len(query_data_list)} queries for user {user.username}"
        )
        ...
        for i, query_data in enumerate(query_data_list):
            try:
                ...
                query, analysis = analyze_query(
                    ...
                    expected_performance=query_data.get(
        ...
        logger.info(
            f"Completed batch analysis: {success_count}/{len(query_data_list)} successful"
        )
