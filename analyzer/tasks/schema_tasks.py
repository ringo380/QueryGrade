...
@shared_task(
    bind=True, max_retries=2, name="analyzer.tasks.analyze_database_schema_async"
)
def analyze_database_schema_async(
    self, db_config: Dict, user_id: int
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying schema analysis in 120 seconds (attempt {self.request.retries + 1})"
            )
