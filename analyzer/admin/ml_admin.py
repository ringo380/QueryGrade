...
from ..models import MLModel, TrainingData, LearningMetrics, FeedbackLearning


@admin.register(MLModel)
class MLModelAdmin(admin.ModelAdmin):
    """Admin interface for MLModel model."""
    list_display = ('name', 'model_type', 'version', 'status', 'performance_display', 'created_at')
    ...
    fieldsets = (
        ...
        ('Performance Metrics', {
            'fields': ('training_accuracy', 'validation_accuracy', 'performance_chart'),
            'classes': ('wide',)
        }),
        ('Training Information', {
            'fields': ('training_samples',),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
            'fields': ('created_at',),
    ...
    def activate_model(self, request, queryset):
        ...
        for obj in queryset:
            MLModel.objects.filter(model_type=obj.model_type).update(status='DEPRECATED')
            obj.status = 'ACTIVE'
            obj.save()
...
class TrainingDataAdmin(admin.ModelAdmin):
    ...
    list_display = ('query_display', 'user_grade_avg', 'user_grade_count', 'system_grade', 'is_validated', 'created_at')
    list_filter = ('is_validated', 'system_grade', 'database_type', ('created_at', DateFieldListFilter))
    ...
    fieldsets = (
        ('Query Information', {
            'fields': ('query', 'database_type', 'query_complexity', 'table_count', 'join_count')
        }),
        ('Grade Information', {
            'fields': ('user_grade_avg', 'user_grade_count', 'user_grade_stddev', 'system_grade', 'system_score')
        }),
        ('Feedback Ratings', {
            'fields': ('accuracy_rating_avg', 'usefulness_rating_avg', 'clarity_rating_avg'),
            'classes': ('collapse',)
        }),
        ('Validation', {
            'fields': ('is_validated', 'validation_source')
        }),
        ('Timestamps', {
...
class LearningMetricsAdmin(admin.ModelAdmin):
    ...
    list_display = ('model', 'accuracy', 'precision', 'recall', 'f1_score', 'avg_user_rating', 'created_at')
    ...
    fieldsets = (
        ...
        ('User Metrics', {
            'fields': ('user_agreement_rate', 'avg_user_rating', 'prediction_count'),
        }),
        ('Operational Metrics', {
...
class FeedbackLearningAdmin(admin.ModelAdmin):
    ...
    list_display = ('user_history_display', 'original_grade', 'original_score', 'feedback_grade_equivalent', 'grade_difference', 'feedback_weight', 'was_used_for_training', 'created_at')
    list_filter = ('was_used_for_training', 'original_grade', ('created_at', DateFieldListFilter))
    search_fields = ('user_history__user__username', 'user_history__query__query_hash', 'training_session')
    ...
    fieldsets = (
        ...
        ('Feedback Impact', {
            'fields': ('feedback_grade_equivalent', 'grade_difference', 'feedback_weight')
        }),
