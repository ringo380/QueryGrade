...
class AntiPatternDetector:
    ...
    def _compile_patterns(self):
        ...
        self.patterns = {
            ...
            'implicit_cross_join': re.compile(r'FROM\s+(\w+)\s*,\s*(\w+)', re.IGNORECASE),
            'or_in_join': re.compile(r'ON\s+.+\s+OR\s+', re.IGNORECASE),
            'functions_in_where': re.compile(r'WHERE\s+.*(YEAR|MONTH|DAY|LOWER|UPPER|SUBSTRING)\s*\([^)]*\w+\.\w+[^)]*\)', re.IGNORECASE),
            ...
            'row_by_row': re.compile(r'WHILE|LOOP|FOR\s+.+\s+IN\s+SELECT', re.IGNORECASE),

            # Correctness anti-patterns
            'null_comparison': re.compile(r'WHERE\s+\w+\s*=\s*NULL|WHERE\s+\w+\s*!=\s*NULL', re.IGNORECASE),
            'double_negative': re.compile(r'NOT\s+.+\s+NOT\s+(IN|EXISTS|LIKE)', re.IGNORECASE),
            'ambiguous_columns': re.compile(r'SELECT\s+(?!.*\.)id|name|status|type(?!\w)', re.IGNORECASE),

            # Security anti-patterns
            'dynamic_sql': re.compile(r'EXEC\s*\(|EXECUTE\s+IMMEDIATE', re.IGNORECASE),
            'string_concat_where': re.compile(r"WHERE\s+.+\+|WHERE\s+.+\|\||WHERE\s+.+CONCAT", re.IGNORECASE),

            # Maintainability anti-patterns
            'magic_numbers': re.compile(r'WHERE\s+.+\s*=\s*\d{3,}(?!\d)', re.IGNORECASE),
            'no_alias': re.compile(r'FROM\s+\w{15,}(?!\s+(?:AS\s+)?\w{1,5}\s)', re.IGNORECASE),
            ...
            'union_instead_union_all': re.compile(r'\bUNION\b(?!\s+ALL)', re.IGNORECASE),
            'distinct_misuse': re.compile(r'SELECT\s+DISTINCT\s+.+\s+FROM\s+.+\s+JOIN', re.IGNORECASE),
        }
    ...
    def detect_anti_patterns(self, query: str) -> AntiPatternReport:
        ...
        performance_risk = self._assess_risk_level(anti_patterns, AntiPatternCategory.PERFORMANCE)
        maintainability_risk = self._assess_risk_level(anti_patterns, AntiPatternCategory.MAINTAINABILITY)
        security_risk = self._assess_risk_level(anti_patterns, AntiPatternCategory.SECURITY)
    ...
    def _calculate_overall_score(self, anti_patterns: List[AntiPattern]) -> float:
        ...
        total_penalty = sum(severity_penalties.get(ap.severity, 0) for ap in anti_patterns)
    ...
    def _assess_risk_level(self, anti_patterns: List[AntiPattern], category: AntiPatternCategory) -> str:
        ...
        high_count = sum(1 for ap in category_patterns if ap.severity == AntiPatternSeverity.HIGH)
        if high_count >= 2:
            return "high"
        elif high_count == 1:
            return "medium"
        ...
        medium_count = sum(1 for ap in category_patterns if ap.severity == AntiPatternSeverity.MEDIUM)
        if medium_count >= 3:
            return "medium"

        return "low"

    def _suggest_rewrite(self, query: str, anti_patterns: List[AntiPattern]) -> Optional[str]:
        ...
        for ap in anti_patterns:
            if ap.severity == AntiPatternSeverity.CRITICAL:
                if ap.pattern_id == "CORR001":  # NULL comparison
                    rewritten = re.sub(r'(\w+)\s*=\s*NULL', r'\1 IS NULL', rewritten, flags=re.IGNORECASE)
                    rewritten = re.sub(r'(\w+)\s*!=\s*NULL', r'\1 IS NOT NULL', rewritten, flags=re.IGNORECASE)
