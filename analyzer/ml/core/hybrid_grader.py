...
class HybridQueryGrader:
    ...
    def analyze_query(self, sql_text: str, database_type: str = '',
                     use_ml: bool = True) -> Tuple[Query, QueryAnalysis]:
        """
        ...
        query, rule_analysis = self.rule_based_grader.analyze_query(sql_text, database_type)
        ...
        try:
            ...
            if ml_prediction is not None:
                ...
                self._log_prediction(query.id, rule_analysis.score, ml_prediction, confidence)
    ...
    def _combine_predictions(self, rule_score: float, ml_score: float,
                           query: Query) -> Tuple[float, str, float]:
        """
        ...
        agreement_factor = max(0, 1 - (score_difference / 50))  # Closer scores = higher confidence
        confidence = (self.model_confidence * ml_weight) + (0.9 * rule_weight) * agreement_factor
    ...
    def train_model(self, force_retrain: bool = False) -> Optional[MLModel]:
        ...
        try:
            ...
            if r2 >= self.performance_threshold:
                saved_model = self._save_model(model, scaler, len(training_data), r2, mse)
                self._update_model_confidence(r2)
                return saved_model
            else:
                logger.warning(f"Model performance too low: R2={r2:.3f} < {self.performance_threshold}")
                return None
    ...
    def _should_retrain(self) -> bool:
        ...
        if active_model.created_at < timezone.now() - timedelta(days=self.retrain_threshold_days):
            return True
    ...
    def _prepare_training_data(self, training_data: List[TrainingData]) -> Tuple[List, List]:
        ...
        for data in training_data:
            try:
                ...
                logger.warning(f"Error extracting features for query {data.query.id}: {str(e)}")
                continue
    ...
    def _save_model(self, model: Any, scaler: Any, training_samples: int,
                   validation_accuracy: float, training_loss: float) -> MLModel:
        ...
        logger.info(f"Saved model: {model_filename} with accuracy {validation_accuracy:.3f}")
        return ml_model
    ...
    def _log_prediction(self, query_id: int, final_score: float,
                       ml_score: float, confidence: float):
