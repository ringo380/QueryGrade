...
class FeatureExtractor:
    ...
    def extract_features(
        self, query: Query, database_type: str = ""
    ) -> Optional[List[float]]:
        """
        ...
        try:
            ...
            features.extend(
                self._extract_database_specific_features(sql_text, database_type)
            )
            ...
            if len(features) != len(self.feature_names):
                logger.error(
                    f"Feature count mismatch: {len(features)} != {len(self.feature_names)}"
                )
                return None
    ...
    def _extract_basic_features(self, sql_text: str, parsed: Statement) -> List[float]:
        ...
        keyword_count = sum(
            1 for token in token_list if token.ttype in sqltokens.Keyword
        )
    ...
    def _extract_complexity_features(
        self, query: Query, parsed: Statement
    ) -> List[float]:
        ...
        aggregate_count = sum(
            sql_text.count(f" {func}(") for func in aggregate_functions
        )
    ...
    def _extract_advanced_features(
        self, sql_text: str, parsed: Statement
    ) -> List[float]:
        ...
        limit_present = any(
            keyword in sql_upper for keyword in ["LIMIT", "TOP", "FETCH"]
        )
    ...
    def _extract_performance_features(
        ...
        cartesian_risk = sql_upper.count("JOIN") > 0 and sql_upper.count(
            "ON "
        ) < sql_upper.count("JOIN")

        # Missing WHERE clause in UPDATE/DELETE
        missing_where = (
            any(keyword in sql_upper for keyword in ["UPDATE ", "DELETE "])
        ...
        where_with_columns = (
            "WHERE" in sql_upper
            and len(re.findall(r"WHERE\s+\w+\s*[=<>]", sql_upper)) > 0
        )
    ...
    def _extract_semantic_features(
        ...
        avg_identifier_length = (
            sum(len(ident) for ident in identifiers) / len(identifiers)
            if identifiers
            else 0
        )
        ...
        keywords = [
            token.value.upper() for token in tokens if token.ttype in sqltokens.Keyword
        ]
        keyword_diversity = len(set(keywords)) / len(keywords) if keywords else 0

        # Count different types of operators
        operators = [
            token.value for token in tokens if token.ttype in sqltokens.Operator
        ]
        comparison_ops = [
        ...
        logical_ops = [
            token.value.upper()
            for token in tokens
            if token.ttype in sqltokens.Keyword
            and token.value.upper() in ["AND", "OR", "NOT"]
        ]
    ...
    def _extract_database_specific_features(
    ...
    def _extract_pattern_features(
        ...
        has_null_check = any(
            ...
            for pattern in ["IS NULL", "IS NOT NULL", "ISNULL", "COALESCE", "NVL"]
        )
