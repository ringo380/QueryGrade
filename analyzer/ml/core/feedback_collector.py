...
class FeedbackCollector:
    ...
    def __init__(self):
        ...
        self.min_feedback_count = 3  # Minimum feedback instances before creating training data
    ...
    def collect_feedback_for_query(self, query_id: int) -> Optional[TrainingData]:
        ...
        try:
            ...
            if len(feedback_data) < self.min_feedback_count:
                logger.debug(f"Insufficient feedback for query {query_id}: {len(feedback_data)} < {self.min_feedback_count}")
                return None
    ...
    def _extract_feedback_from_history(self, history: UserQueryHistory) -> Optional[Dict]:
        ...
        try:
            ...
            # Check for simple thumbs up/down feedback
                ...
                user_grade = 4.0 if history.was_helpful else 2.0  # Convert boolean to 1-5 scale
            ...
            logger.warning(f"Error extracting feedback from history {history.id}: {str(e)}")
            return None

    def _convert_ratings_to_grade(self, accuracy: Optional[int],
                                 usefulness: Optional[int],
                                 clarity: Optional[int]) -> float:
    ...
    def _calculate_feedback_confidence(self, feedback: QueryFeedback) -> float:
        ...
        rating_count = sum(1 for r in [feedback.accuracy_rating, feedback.usefulness_rating, feedback.clarity_rating] if r is not None)
        confidence += (rating_count / 3) * 0.3

        # Higher confidence for consistent ratings
        ratings = [r for r in [feedback.accuracy_rating, feedback.usefulness_rating, feedback.clarity_rating] if r is not None]
        if len(ratings) > 1:
            rating_variance = sum((r - sum(ratings)/len(ratings))**2 for r in ratings) / len(ratings)
            consistency_bonus = max(0, (2.0 - rating_variance) / 2.0) * 0.2  # Lower variance = higher consistency
    ...
    def _aggregate_feedback(self, feedback_data: List[Dict]) -> Dict:
        ...
        variance = sum(((feedback['user_grade'] - avg_grade) ** 2) *
                      (self._get_user_reliability(feedback['user']) * feedback['confidence'])
                      for feedback in feedback_data) / total_weight
        stddev = variance ** 0.5
        ...
        return {
            ...
            'accuracy_rating_avg': sum(accuracy_ratings) / len(accuracy_ratings) if accuracy_ratings else None,
            'usefulness_rating_avg': sum(usefulness_ratings) / len(usefulness_ratings) if usefulness_ratings else None,
            'clarity_rating_avg': sum(clarity_ratings) / len(clarity_ratings) if clarity_ratings else None,
    ...
    def _get_user_reliability(self, user: User) -> float:
        ...
        try:
            ...
            feedback_count = QueryFeedback.objects.filter(user_history__user=user).count()
    ...
    @transaction.atomic
    def _create_training_data(self, query: Query, analysis: QueryAnalysis,
                             aggregated: Dict) -> TrainingData:
        ...
        try:
            ...
            if not created:
                ...
                training_data.accuracy_rating_avg = aggregated.get('accuracy_rating_avg')
                training_data.usefulness_rating_avg = aggregated.get('usefulness_rating_avg')
                training_data.clarity_rating_avg = aggregated.get('clarity_rating_avg')
                training_data.save()

            logger.info(f"{'Created' if created else 'Updated'} training data for query {query.id}")
            return training_data
    ...
    def _create_learning_records(self, user_histories: List[UserQueryHistory],
                               analysis: QueryAnalysis, aggregated: Dict):
        ...
        for history in user_histories:
            try:
                ...
                logger.warning(f"Error creating learning record for history {history.id}: {str(e)}")
    ...
    def batch_collect_feedback(self, days_back: int = 7) -> List[TrainingData]:
        ...
        logger.info(f"Collected feedback for {len(training_data_list)} queries from last {days_back} days")
        return training_data_list

    def get_training_dataset(self, min_feedback_count: int = 3,
                           include_validated_only: bool = False) -> List[TrainingData]:
        """Get all training data suitable for model training."""
        queryset = TrainingData.objects.filter(
            user_grade_count__gte=min_feedback_count
        )
