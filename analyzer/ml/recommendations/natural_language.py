...
class NaturalLanguageFeedbackGenerator:
    ...
    def generate_feedback(self, analysis_results: Dict[str, Any]) -> ComprehensiveFeedback:
    ...
    def _generate_summary(self, grade: str, score: float, analysis: Dict[str, Any]) -> str:
        ...
        if 'query_type' in analysis:
            summary_parts.append(f"This {analysis['query_type']} query scores {score:.1f}/100.")

        # Major issues
        anti_patterns = analysis.get('anti_patterns', [])
        critical_count = sum(1 for ap in anti_patterns if ap.get('severity') == 'critical')
        high_count = sum(1 for ap in anti_patterns if ap.get('severity') == 'high')

        if critical_count > 0:
            summary_parts.append(f"Found {critical_count} critical issue(s) requiring immediate attention.")
        elif high_count > 0:
            summary_parts.append(f"Identified {high_count} high-priority optimization opportunities.")

        # Performance estimate
        if 'estimated_execution_time' in analysis:
            exec_time = analysis['estimated_execution_time']
            if exec_time > 1000:
                summary_parts.append(f"Current estimated execution time: {exec_time/1000:.1f} seconds.")
            else:
                summary_parts.append(f"Current estimated execution time: {exec_time:.0f}ms.")
    ...
    def _generate_improvements(self, anti_patterns: List[Dict[str, Any]],
                              optimizations: List[Dict[str, Any]]) -> List[FeedbackMessage]:
        ...
        for ap in anti_patterns:
            message = self._create_feedback_message(
                ...
                recommendation=ap.get('recommendation', 'Consider reviewing this pattern'),
    ...
    def _create_feedback_message(self, **kwargs) -> FeedbackMessage:
        ...
        if self.user_level == FeedbackLevel.BEGINNER:
            kwargs['description'] = self._simplify_language(kwargs.get('description', ''))
    ...
    def _is_easy_fix(self, improvement: FeedbackMessage) -> bool:
        ...
        easy_patterns = [
            'SELECT *', 'Missing LIMIT', 'Missing alias',
            'Inconsistent case', 'Magic numbers'
        ]
    ...
    def _generate_long_term_suggestions(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis.get('avg_query_complexity', 0) > 7:
            suggestions.append("📚 Consider breaking complex queries into smaller, maintainable parts using CTEs or views")

        if analysis.get('subquery_count', 0) > 3:
            suggestions.append("🔄 Refactor nested subqueries into JOINs or CTEs for better performance")

        if not analysis.get('uses_modern_syntax', True):
            suggestions.append("🎓 Migrate to modern SQL syntax (e.g., explicit JOINs instead of comma-separated tables)")

        if analysis.get('missing_indexes_count', 0) > 2:
            suggestions.append("📊 Conduct a comprehensive index analysis and create a indexing strategy")

        if analysis.get('scalability_score', 100) < 70:
            suggestions.append("📈 Design queries with future data growth in mind - current approach may not scale")

        # Add learning suggestions based on patterns
        if analysis.get('repeated_mistakes', []):
            suggestions.append("📖 Focus on learning about: " + ", ".join(analysis['repeated_mistakes'][:3]))
    ...
    def _select_encouragement(self, grade: str) -> str:
        ...
        messages = self.encouragement_templates.get(grade, self.encouragement_templates['C'])
        return random.choice(messages)

    def _generate_next_steps(self, improvements: List[FeedbackMessage], grade: str) -> List[str]:
        ...
        if len(next_steps) < 5:
            next_steps.append("5️⃣ Test changes with EXPLAIN PLAN to verify improvements")
    ...
    def _generate_comparative_analysis(self, analysis: Dict[str, Any]) -> Optional[str]:
        ...
        if 'performance_delta' in baseline:
            ...
            if delta > 0:
                comparison_parts.append(f"📈 {delta:.0f}% faster than average similar queries")
            else:
                comparison_parts.append(f"📉 {abs(delta):.0f}% slower than average similar queries")

        # Complexity comparison
        if 'complexity_percentile' in baseline:
            percentile = baseline['complexity_percentile']
            if percentile > 75:
                comparison_parts.append(f"More complex than {percentile:.0f}% of similar queries")
            elif percentile < 25:
                comparison_parts.append(f"Simpler than {100-percentile:.0f}% of similar queries")
    ...
    def _estimate_improvement(self, improvements: List[FeedbackMessage]) -> Optional[str]:
        ...
        if total_impact > 0.5:
            ...
            return f"Moderate performance improvement of {total_impact*100:.0f}% expected"
        else:
            return f"Minor performance gains of approximately {total_impact*100:.0f}%"

    def _compile_learning_resources(self, improvements: List[FeedbackMessage],
        ...
        topic_map = {
            'index': {'title': 'Database Indexing Fundamentals', 'url': '#indexing', 'level': 'intermediate'},
            'join': {'title': 'SQL JOIN Types and Optimization', 'url': '#joins', 'level': 'intermediate'},
            'subquery': {'title': 'Subqueries vs JOINs', 'url': '#subqueries', 'level': 'advanced'},
            'performance': {'title': 'SQL Performance Tuning', 'url': '#performance', 'level': 'advanced'},
            'security': {'title': 'SQL Injection Prevention', 'url': '#security', 'level': 'intermediate'},
            'null': {'title': 'Understanding NULL in SQL', 'url': '#null-handling', 'level': 'beginner'},
            'aggregate': {'title': 'Aggregation Functions and GROUP BY', 'url': '#aggregation', 'level': 'intermediate'},
            'window': {'title': 'Window Functions', 'url': '#window-functions', 'level': 'advanced'}
        }
                ...
                if keyword in imp.title.lower() and keyword not in topics_covered:
                    ...
                    if self._is_appropriate_level(resource['level']):
                        resources.append({
                            ...
                            'url': resource['url'],
                            'relevance': 'High' if imp.severity in ['critical', 'high'] else 'Medium'
                        })
...
def format_feedback_as_markdown(feedback: ComprehensiveFeedback) -> str:
    ...
    lines.append(f"## Overall Grade: **{feedback.overall_grade}** ({feedback.overall_score:.1f}/100)")
    lines.append("")
    ...
    if feedback.learning_resources:
        ...
        for resource in feedback.learning_resources:
            lines.append(f"- [{resource['title']}]({resource['url']}) - Relevance: {resource['relevance']}")
        lines.append("")
