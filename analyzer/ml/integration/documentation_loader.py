...
class DocumentationLoader:
    ...
    def load_all_documentation(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        for source in self.sources:
            ...
            try:
                ...
                results['benchmarks_loaded'] += len(source_results.get('benchmarks', []))
    ...
    def _load_web_documentation(self, source: DocumentationSource, force_refresh: bool) -> Dict[str, Any]:
    ...
    def _fetch_web_content(self, url: str) -> Optional[str]:
        ...
        try:
            headers = {
                'User-Agent': 'QueryGrade Documentation Loader 1.0'
            }
    ...
    def _parse_documentation_content(self, content: str, source: DocumentationSource,
                                   source_url: str) -> Tuple[List[DocumentationRule], List[BenchmarkResult]]:
        ...
        benchmarks.extend(self._extract_benchmarks_from_content(soup, source, source_url))

        return rules, benchmarks

    def _parse_mysql_documentation(self, soup: BeautifulSoup, source: DocumentationSource,
                                 source_url: str) -> List[DocumentationRule]:
        """Parse MySQL-specific documentation."""
        rules = []

        # Look for optimization recommendations
        optimization_sections = soup.find_all(['div', 'section'], class_=re.compile(r'note|tip|important'))

        for section in optimization_sections:
            text = section.get_text().strip()

            # Extract SELECT * warnings
            if 'SELECT *' in text.upper() and ('avoid' in text.lower() or 'performance' in text.lower()):
                rules.append(DocumentationRule(
            ...
            if 'index' in text.lower() and ('create' in text.lower() or 'use' in text.lower()):
                rules.append(DocumentationRule(
    ...
    def _parse_postgresql_documentation(self, soup: BeautifulSoup, source: DocumentationSource,
                                      source_url: str) -> List[DocumentationRule]:
    ...
    def _parse_sqlite_documentation(self, soup: BeautifulSoup, source: DocumentationSource,
                                  source_url: str) -> List[DocumentationRule]:
    ...
    def _parse_generic_documentation(self, soup: BeautifulSoup, source: DocumentationSource,
                                   source_url: str) -> List[DocumentationRule]:
    ...
    def _extract_benchmarks_from_content(self, soup: BeautifulSoup, source: DocumentationSource,
                                       source_url: str) -> List[BenchmarkResult]:
        """Extract benchmark queries from documentation examples."""
        benchmarks = []

        # Find code blocks with SQL
        code_blocks = soup.find_all(['code', 'pre'], string=re.compile(r'SELECT|INSERT|UPDATE|DELETE', re.I))
    ...
    def _parse_json_documentation(self, data: Dict[str, Any], source: DocumentationSource) -> List[DocumentationRule]:
        ...
        if 'rules' in data:
            for rule_data in data['rules']:
                rules.append(DocumentationRule(
                    ...
                    database_type=rule_data.get('database_type', source.database_type),
                ...
                if benchmark.validated:
                    ...
                    query, created = Query.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'query_type': self._extract_query_type(benchmark.query_text),
                            ...
                            'where_conditions': benchmark.query_text.upper().count('WHERE'),
    ...
    def get_documentation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_update': max([s.last_updated for s in self.sources if s.last_updated], default=None)
        }
    ...
    def validate_benchmarks(self, sample_size: int = 10) -> Dict[str, Any]:
        ...
        sample_benchmarks = random.sample(
            self.benchmarks,
            min(sample_size, len(self.benchmarks))
        )
        ...
        for benchmark in sample_benchmarks:
            ...
            if 0 <= benchmark.expected_score <= 100:
                ...
                total_error += abs(benchmark.expected_score - 50)  # Assume 50 as baseline

        accuracy = validated_count / len(sample_benchmarks) if sample_benchmarks else 0.0

        return {
            'validated': validated_count,
            'total': len(sample_benchmarks),
            'accuracy': accuracy,
            'average_error': total_error / len(sample_benchmarks) if sample_benchmarks else 0.0
        }