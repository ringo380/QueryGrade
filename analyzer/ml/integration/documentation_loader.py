    ...
    def _parse_mysql_documentation(
        ...
        for section in optimization_sections:
            ...
            if "SELECT *" in text.upper() and (
                ...
                rules.append(
                    DocumentationRule(
                        rule_id=f"mysql_select_star_{hashlib.md5(text.encode(), usedforsecurity=False).hexdigest()[:8]}",
    ...
    def _parse_postgresql_documentation(
        ...
        for section in tip_sections:
            ...
            if "EXPLAIN" in text.upper() and "performance" in text.lower():
                rules.append(
                    DocumentationRule(
                        rule_id=f"postgresql_explain_{hashlib.md5(text.encode(), usedforsecurity=False).hexdigest()[:8]}",
    ...
    def _extract_benchmarks_from_content(
        ...
        for block in code_blocks:
            ...
            if self._is_valid_sql_example(sql_text):
                ...
                benchmarks.append(
                    BenchmarkResult(
                        benchmark_id=f"benchmark_{hashlib.md5(sql_text.encode(), usedforsecurity=False).hexdigest()[:8]}",
                        query_text=sql_text,
