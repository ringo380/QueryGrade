...
class QueryPatternGenerator:
    ...
    def __init__(self):
        ...
        self.sample_tables = ['users', 'orders', 'products', 'customers', 'employees', 'departments']
        self.sample_columns = ['id', 'name', 'email', 'created_at', 'status', 'price', 'quantity']
        self.sample_conditions = ['id > 100', 'status = "active"', 'created_at > "2023-01-01"']
    ...
    def generate_anti_pattern_queries(self, anti_pattern_type: str, count: int = 5) -> List[str]:
    ...
    def _substitute_pattern(self, pattern: str) -> str:
        ...
        substitutions = {
            '{columns}': random.choice(['*', 'id', 'name, email', 'COUNT(*)', 'id, name']),
            ...
            '{condition2}': random.choice(['price > 50', 'quantity < 100', 'name LIKE "%test%"']),
...
class BenchmarkGenerator:
    ...
    def generate_comprehensive_benchmarks(self, database_types: List[str] = None) -> List[BenchmarkSet]:
    ...
    def _generate_pattern_benchmarks(self, database_type: str) -> List[BenchmarkQuery]:
        ...
        for complexity in ['simple', 'medium', 'complex', 'advanced']:
            queries = self.pattern_generator.generate_pattern_queries(complexity, count=10)
        ...
        for anti_type in ['performance', 'syntax']:
            queries = self.pattern_generator.generate_anti_pattern_queries(anti_type, count=5)
    ...
    def _generate_documentation_benchmarks(self, database_type: str) -> List[BenchmarkQuery]:
        ...
        try:
            ...
            logger.warning(f"Error generating documentation benchmarks for {database_type}: {e}")

        return benchmarks

    def _generate_performance_benchmarks(self, database_type: str) -> List[BenchmarkQuery]:
    ...
    def save_benchmark_set(self, benchmark_set: BenchmarkSet) -> str:
        ...
        logger.info(f"Saved benchmark set with {len(benchmark_set.benchmarks)} benchmarks to {filepath}")
        return filepath
    ...
    def create_training_data_from_benchmarks(self, benchmark_sets: List[BenchmarkSet]) -> int:
                    ...
                    try:
                        ...
                        query, created = Query.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                'query_type': self._get_query_type(benchmark.query_text),
                                'estimated_complexity': self._estimate_complexity(benchmark.query_text)
                            }
                        )
                        ...
                        training_data, created = TrainingData.objects.get_or_create(
                            query=query,
                            defaults={
                                'user_grade_avg': (benchmark.expected_score / 100) * 5,  # Convert to 1-5 scale
                                'user_grade_count': 1,
                                'user_grade_stddev': 0.1,
                                'system_grade': benchmark.expected_grade,
                                'system_score': benchmark.expected_score,
                                'query_complexity': self._estimate_complexity(benchmark.query_text),
                        ...
                        logger.warning(f"Error creating training data for benchmark query: {e}")
...
def generate_default_benchmarks():
    ...
    logger.info(f"Generated {len(benchmark_sets)} benchmark sets with {training_count} training examples")
