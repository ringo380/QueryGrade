...
def dashboard_api_overview(request):
    ...
    try:
        ...
        # User engagement
        active_users = QueryFeedback.objects.filter(
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'performance_metrics': {
                    ...
                    'model_accuracy': round(model_accuracy * 100, 1) if model_accuracy else 0,
                },
...
def dashboard_api_models(request):
    ...
    try:
        ...
        for model in MLModel.objects.all().order_by('-created_at'):
            ...
            user_satisfaction = recent_feedback.aggregate(
                avg=Avg('user_feedback_score')
            )['avg'] or 0
    ...
    try:
        ...
        for i in range(30):
            ...
            satisfaction_trend.append({
                ...
                'satisfaction': round(daily_satisfaction, 2) if daily_satisfaction else None
            })

        # Top contributors
        top_contributors = QueryFeedback.objects.values('user__username').annotate(
            feedback_count=Count('id'),
            avg_satisfaction=Avg('score_agreement')
...
def dashboard_api_realtime(request):
    ...
    try:
        ...
        # Recent activity
        recent_activity = {
            'queries_last_hour': Query.objects.filter(created_at__gte=one_hour_ago).count(),
            'feedback_last_hour': QueryFeedback.objects.filter(created_at__gte=one_hour_ago).count(),
...
def _get_training_recommendations(training_status: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    if latest_model['created_at']:
        model_age = (timezone.now() - datetime.fromisoformat(latest_model['created_at'])).days
        if model_age > 30:
            recommendations.append({
                'type': 'info',
                'title': 'Model Age',
