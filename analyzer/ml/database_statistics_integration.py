...
class DatabaseStatisticsManager:
        ...
        if 'tables' in data:
            for table_data in data['tables']:
                table_stat = TableStatistics(
                    ...
                    last_analyzed=datetime.fromisoformat(table_data.get('last_analyzed', datetime.now().isoformat())),
        ...
        if 'columns' in data:
            for col_data in data['columns']:
                col_stat = ColumnStatistics(
                    ...
                    distribution=DataDistribution(col_data.get('distribution', 'uniform'))
                )
    ...
    def estimate_selectivity(self, table_name: str, column_name: str, operator: str, value: Any) -> float:
        ...
        if operator == '=':
            ...
            if col_stats.min_value is not None and col_stats.max_value is not None:
                try:
                    range_fraction = (value - col_stats.min_value) / (col_stats.max_value - col_stats.min_value)
            ...
            if col_stats.min_value is not None and col_stats.max_value is not None:
                try:
                    range_fraction = (col_stats.max_value - value) / (col_stats.max_value - col_stats.min_value)
                    return max(0.0, min(1.0, range_fraction))
                except:
                    return 0.3
            return 0.3
    ...
    def estimate_join_cardinality(self, left_table: str, right_table: str,
                                 join_column_left: str, join_column_right: str) -> int:
        ...
        for rel in self.relationship_stats:
            if (rel.parent_table == left_table and rel.child_table == right_table and
                rel.parent_column == join_column_left and rel.child_column == join_column_right):
        ...
        if left_col_stats and right_col_stats:
            ...
            join_factor = min(left_col_stats.distinct_values, right_col_stats.distinct_values)
            if join_factor > 0:
    ...
    def suggest_index(self, table_name: str, columns: List[str], query_pattern: str = None) -> Dict[str, Any]:
        ...
        for index in existing_indexes:
            if set(columns).issubset(set(index.columns)):
                suggestion['reasoning'].append("Columns already covered by existing index")
        ...
        if query_pattern and self.workload_stats:
            ...
            if pattern_frequency > 100:
                ...
                suggestion['reasoning'].append(f"Frequently used in {query_pattern} queries")
    ...
    def analyze_data_skew(self, table_name: str, column_name: str) -> Dict[str, Any]:
        ...
        if col_stats.most_common_values:
            ...
            if top_value_frequency > 0.5:
                ...
                analysis['recommendations'].append("Consider filtered statistics or indexes")
        ...
        if col_stats.null_percentage > 0.7:
            ...
            analysis['recommendations'].append("Consider filtered index excluding NULLs")
        ...
        if col_stats.histogram_buckets:
            ...
            if bucket_sizes:
                ...
                if cv > 1.0:  # Coefficient of variation > 1 indicates high skew
                    ...
                    analysis['recommendations'].append("Update statistics with higher sampling rate")

        return analysis

    def get_correlated_columns(self, table_name: str, column_name: str,
        ...
        for other_column, other_stats in self.column_stats.get(table_name, {}).items():
            ...
            if target_stats.distinct_values > 0 and other_stats.distinct_values > 0:
                cardinality_ratio = min(target_stats.distinct_values, other_stats.distinct_values) / \
        ...
        if 'ORDER BY' in str(query_analysis):
            ...
            for table in query_analysis.get('tables', []):
                if table in self.table_stats:
                    estimated_rows = min(estimated_rows, self.table_stats[table].row_count)
        ...
        if 'joins' in query_analysis:
            for join in query_analysis['joins']:
                ...
                if 'hash' in str(join).lower():
                    ...
                    memory_mb += build_table_rows * 0.0002  # 0.2KB per row for hash table
    ...
    def get_partition_elimination_hints(self, table_name: str, filters: List[Dict[str, Any]]) -> List[str]:
        ...
        for filter_info in filters:
            ...
            if 'date' in filter_info.get('column', '').lower():
                hints.append(f"Partition elimination possible on {filter_info['column']}")

        return hints

    def track_query_performance(self, query_hash: str, execution_time_ms: float,
                               rows_returned: int, cpu_time_ms: float = 0):
    ...
    def get_query_performance_trend(self, query_hash: str) -> Dict[str, Any]:
        ...
        if perf['avg_time_ms'] > 0:
            volatility = (perf['max_time_ms'] - perf['min_time_ms']) / perf['avg_time_ms']
    ...
    def export_statistics(self, output_path: str, format: str = 'json'):
        ...
        try:
            if format == 'json':
                data = {
                    'tables': [self._table_stats_to_dict(ts) for ts in self.table_stats.values()],
                    'indexes': [self._index_stats_to_dict(idx)
                              for idx_list in self.index_stats.values()
                              for idx in idx_list],
                    'columns': [self._column_stats_to_dict(cs)
...
def generate_context_aware_features(query: str, stats_manager: DatabaseStatisticsManager) -> Dict[str, Any]:
    """
    ...
    if where_match:
        ...
        for condition in conditions:
            ...
            match = re.match(r'(\w+\.)?(\w+)\s*([=<>]+|LIKE|IN|BETWEEN)\s*(.+)', condition, re.IGNORECASE)
            if match:
                table_alias, column, operator, value = match.groups()
                # Estimate selectivity
                for table in tables:
                    selectivity = stats_manager.estimate_selectivity(table, column, operator, value)
