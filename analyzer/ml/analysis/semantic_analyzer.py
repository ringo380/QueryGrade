...
class SemanticMetrics:
    ...
    subquery_types_distribution: Dict[str, int] = field(
    ...
    join_types_distribution: Dict[str, int] = field(
        default_factory=dict
    )  # Type breakdown
    join_impacts_distribution: Dict[str, int] = field(
        default_factory=dict
    )  # Impact breakdown
    ...
    cte_purposes_distribution: Dict[str, int] = field(
        default_factory=dict
    )  # Purpose breakdown
    ...
    statement_types_distribution: Dict[str, int] = field(
        default_factory=dict
    )  # Type breakdown
    ...
    goal_optimization_recommendations: List[str] = field(
    ...
    def overall_score(self) -> float:
        ...
        complexity_score = (
            ...
            + (1.0 - self.maintenance_difficulty)
            * 0.2  # Lower difficulty = higher score
...
class SemanticFeatureExtractor:
    ...
    def _compile_patterns(self):
        ...
        self.patterns = {
            ...
            "temporal_comparisons": re.compile(
                r'\b(BETWEEN.*AND|>=|<=|>|<)\s+[\'"]*\d{4}-\d{2}-\d{2}', re.IGNORECASE
            ),
            # Analytical patterns
            ...
            "advanced_aggregates": re.compile(
                r"\b(STDDEV|VARIANCE|PERCENTILE|MEDIAN|MODE)\s*\(", re.IGNORECASE
            ),
            # Set operations
            ...
            "hierarchical_functions": re.compile(
                r"\b(CONNECT\s+BY|START\s+WITH|LEVEL)\b", re.IGNORECASE
            ),
            # Performance indicators
            "force_index": re.compile(
                r"\b(FORCE\s+INDEX|USE\s+INDEX|IGNORE\s+INDEX)\b", re.IGNORECASE
            ),
            "hints": re.compile(r"/\*\+.*?\*/", re.DOTALL),
        }
    ...
    def _analyze_nested_subqueries(
        self, query: str, parsed: Statement, metrics: SemanticMetrics
    ):
        ...
        try:
            ...
            if analysis.max_nesting_depth >= 3:
                metrics.conceptual_complexity = min(
                    1.0, metrics.conceptual_complexity + 0.3
                )
            elif analysis.max_nesting_depth >= 2:
                metrics.conceptual_complexity = min(
                    1.0, metrics.conceptual_complexity + 0.15
                )

            # Adjust maintenance difficulty based on subquery complexity
            if analysis.correlated_count > 0:
                metrics.maintenance_difficulty = min(
                    1.0, metrics.maintenance_difficulty + 0.25
                )

            if analysis.total_subquery_count > 5:
                metrics.maintenance_difficulty = min(
                    1.0, metrics.maintenance_difficulty + 0.15
                )
    ...
    def _analyze_joins(self, query: str, parsed: Statement, metrics: SemanticMetrics):
        ...
        try:
            ...
            if analysis.total_join_count > 0:
                ...
                metrics.conceptual_complexity = min(
                    1.0,
                    metrics.conceptual_complexity
                    + analysis.overall_complexity_score * 0.2,
                )
            ...
            if analysis.has_implicit_joins:
                ...
                metrics.maintenance_difficulty = min(
                    1.0, metrics.maintenance_difficulty + 0.2
                )

            # CROSS JOINs indicate high risk
            if analysis.cross_join_count > 0:
                metrics.conceptual_complexity = min(
                    1.0, metrics.conceptual_complexity + 0.25
                )
                metrics.maintenance_difficulty = min(
                    1.0, metrics.maintenance_difficulty + 0.3
                )
        ...
        try:
            ...
            if analysis.total_cte_count > 0:
                metrics.conceptual_complexity = min(
                    1.0,
                    metrics.conceptual_complexity
                    + analysis.overall_complexity_score * 0.15,
                )
                metrics.cognitive_load = min(
                    1.0,
                    metrics.cognitive_load + analysis.overall_complexity_score * 0.1,
                )

            # Recursive CTEs add complexity
            if analysis.recursive_cte_count > 0:
                metrics.conceptual_complexity = min(
                    1.0, metrics.conceptual_complexity + 0.2
                )
                metrics.maintenance_difficulty = min(
                    1.0, metrics.maintenance_difficulty + 0.25
                )
    ...
    def _analyze_context_window(
        ...
        try:
            ...
            metrics.execution_mode_recommended = (
            ...
            if metrics.is_multi_statement:
                ...
                metrics.conceptual_complexity = min(
                    1.0,
                    metrics.conceptual_complexity
                    + analysis.overall_complexity_score * 0.1,
                )
                metrics.cognitive_load = min(
                    1.0,
                    metrics.cognitive_load + analysis.overall_complexity_score * 0.15,
                )
            ...
            if len(analysis.data_flows) > 0:
                metrics.conceptual_complexity = min(
                    1.0, metrics.conceptual_complexity + len(analysis.data_flows) * 0.05
                )

        except Exception as e:
            self.logger.warning(f"Error analyzing context window: {e}")

    def _analyze_query_goals(
        self, query: str, parsed: Statement, metrics: SemanticMetrics
    ):
        ...
        try:
            ...
            metrics.goal_optimization_recommendations = (
            ...
            if analysis.primary_goal.value == "reporting":
                ...
                metrics.aggregation_complexity = min(
                    1.0, metrics.aggregation_complexity * 0.8
                )
    ...
    def _analyze_query_intent(
        ...
        if any(func in query_upper for func in ["GROUP BY", "HAVING", "PARTITION BY"]):
            analytical_score += 2
        if any(
            func in query_upper for func in ["SUM(", "COUNT(", "AVG(", "MAX(", "MIN("]
        ):
            analytical_score += 1
        ...
        if max(scores.values()) == 0:
            ...
            for intent, score in sorted_intents[1:]:
                if (
                    score > 0 and score >= total_score * 0.2
                ):  # At least 20% of total score
    ...
    def _analyze_relational_operations(
        self, query: str, parsed: Statement, metrics: SemanticMetrics
    ):
        ...
        metrics.projection_complexity = min(
            1.0,
            (
                select_items * 0.1
        ...
        if "WHERE" in query_upper:
            where_clause = (
                query.split("WHERE", 1)[1].split("GROUP BY")[0]
                if "GROUP BY" in query
                else query.split("WHERE", 1)[1]
            )
            logical_operators = len(
                re.findall(r"\b(AND|OR)\b", where_clause, re.IGNORECASE)
            )
            ...
            exists_operators = len(
                re.findall(r"\bEXISTS\b", where_clause, re.IGNORECASE)
            )

            metrics.selection_complexity = min(
                1.0,
                (
        ...
        aggregate_functions = len(
            re.findall(r"\b(COUNT|SUM|AVG|MAX|MIN|STDDEV|VARIANCE)\s*\(", query_upper)
        )
        ...
        if order_by_used:
            ...
            metrics.sorting_complexity = min(
                1.0, 0.2 + order_by_columns * 0.1 + (0.1 if desc_used else 0)
            )
    ...
    def _analyze_data_flow(
        ...
        from_match = re.search(
            r"\bFROM\s+([^WHERE^GROUP^ORDER^LIMIT^UNION^INTERSECT^EXCEPT]+)",
            query_upper,
        )
        if from_match:
    ...
    def _analyze_access_patterns(
        ...
        if max(pattern_scores.values()) == 0:
            ...
            metrics.access_pattern_confidence = (
                pattern_scores[metrics.primary_access_pattern] / total_score
            )
    ...
    def _analyze_complexity_indicators(
        ...
        conceptual_factors = [
            len(re.findall(r"\bJOIN\b", query_upper))
            * 0.1,  # Joins add conceptual complexity
            (1 if "GROUP BY" in query_upper else 0)
            * 0.2,  # Aggregation adds complexity
            len(re.findall(r"\bSELECT\b", query_upper))
            * 0.15,  # Subqueries add complexity
            len(re.findall(r"\bCASE\b", query_upper)) * 0.1,  # Conditional logic
            (
                1
                if any(op in query_upper for op in ["UNION", "INTERSECT", "EXCEPT"])
                else 0
            )
            * 0.3,
        ]
        ...
        maintenance_factors = [
            len(re.findall(r"\bSELECT\b", query_upper))
            * 0.1,  # Subqueries make maintenance harder
    ...
    def _analyze_advanced_features(
        self, query: str, parsed: Statement, metrics: SemanticMetrics
    ):
        ...
        metrics.temporal_complexity = min(
            1.0, (temporal_functions + temporal_comparisons) * 0.2
        )

        # Hierarchical complexity
        recursive_cte = len(self.patterns["recursive_cte"].findall(query))
        hierarchical_functions = len(
            self.patterns["hierarchical_functions"].findall(query)
        )
        metrics.hierarchical_complexity = min(
            1.0, (recursive_cte + hierarchical_functions) * 0.5
        )
    ...
    def _predict_performance_characteristics(
        self, query: str, parsed: Statement, metrics: SemanticMetrics
    ):
        ...
        index_indicators = [
            ...
            ),  # Specific columns more likely to use indexes
        ]
        ...
        parallel_indicators = [
            ...
            len(re.findall(r"\bJOIN\b", query_upper))
            >= 2,  # Complex joins benefit from parallelism
        ...
        metrics.parallel_execution_potential = sum(parallel_indicators) / len(
            parallel_indicators
        )
...
def analyze_query_semantics(
    ...
    return {
        ...
        "access_patterns": {
            ...
            "secondary_patterns": [
                pattern.value for pattern in metrics.secondary_patterns
            ],
        },
