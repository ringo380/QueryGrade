...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        subparsers = parser.add_subparsers(
            dest="action", help="Available analytics actions"
        )
        ...
        performance_parser = subparsers.add_parser(
            "model-performance", help="Analyze model performance"
        )
        performance_parser.add_argument(
            "--model-version", type=str, help="Analyze specific model version"
        )
        performance_parser.add_argument(
            "--days", type=int, default=30, help="Analysis period in days (default: 30)"
        )
        ...
        satisfaction_parser = subparsers.add_parser(
            "user-satisfaction", help="Analyze user satisfaction metrics"
        )
        ...
        export_parser = subparsers.add_parser(
            "export-metrics", help="Export metrics to file"
        )
        ...
        export_parser.add_argument(
            "--days", type=int, default=30, help="Period to export (default: 30 days)"
        )

        # Model comparison command
        comparison_parser = subparsers.add_parser(
            "model-comparison", help="Compare model versions"
        )
        comparison_parser.add_argument(
            "versions", nargs="+", help="Model versions to compare"
        )
    ...
    def _show_system_overview(self):
        ...
        total_users = (
            User.objects.filter(queryfeedback__isnull=False).distinct().count()
        )
    ...
    def _show_recent_activity(self):
        ...
        for i in range(7):
            ...
            day_queries = Query.objects.filter(created_at__date=day.date()).count()
            ...
            if day_queries > 0 or day_feedback > 0:
                self.stdout.write(
                    f'  {day.strftime("%Y-%m-%d")}: {day_queries} queries, {day_feedback} feedback'
                )
    ...
    def _show_performance_summary(self):
        ...
        if latest_metrics:
            ...
            self.stdout.write(
                f"  Training accuracy: {latest_metrics.training_accuracy:.1%}"
            )
            self.stdout.write(
                f"  Validation accuracy: {latest_metrics.validation_accuracy:.1%}"
            )
            self.stdout.write(
                f"  Feedback correlation: {latest_metrics.feedback_correlation:.3f}"
            )
            self.stdout.write(
                f"  User satisfaction: {latest_metrics.user_satisfaction_avg:.1f}/5.0"
            )
    ...
    def _show_alerts_and_recommendations(self):
        ...
        for model in MLModel.objects.filter(is_active=True):
            ...
            if age > 30:
                recommendations.append(
                    f"Consider retraining {model.version} (age: {age} days)"
                )
        ...
        if training_data_count < 100:
            recommendations.append(
                f"Low training data: {training_data_count} samples (recommended: 100+)"
            )
        ...
        if recent_feedback < 10:
            recommendations.append(
                f"Low recent feedback volume: {recent_feedback} items this week"
            )
    ...
    def _analyze_single_model(self, model, days):
        ...
        self.stdout.write(
            f'Validation accuracy: {metrics.get("validation_accuracy", "N/A")}'
        )
        ...
        recent_feedback = FeedbackLearning.objects.filter(
            model_version=model.version, created_at__gte=cutoff_date
        )

        if recent_feedback.exists():
            avg_agreement = recent_feedback.aggregate(avg=Avg("user_feedback_score"))[
                "avg"
            ]
            self.stdout.write(
    ...
    def analyze_features(self, options):
        ...
        if (
            not active_model
            or "feature_importance" not in active_model.performance_metrics
        ):
            self.stdout.write(
                self.style.WARNING("No feature importance data available")
            )
            return
    ...
    def _analyze_user_engagement(self):
        ...
        top_contributors = (
            ...
            .annotate(feedback_count=Count("queryfeedback"))
    ...
    def compare_models(self, options):
        ...
        # Compare metrics
        self.stdout.write(
            f'{"Metric":<25} {" | ".join(f"{m.version[:15]:<15}" for m in models)}'
        )
