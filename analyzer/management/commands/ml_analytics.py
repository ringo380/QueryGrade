...
class Command(BaseCommand):
    ...
    def _show_performance_summary(self):
        ...
        if latest_metrics:
            ...
            self.stdout.write(
                f"  User agreement rate: {latest_metrics.user_agreement_rate:.3f}"
            )
            self.stdout.write(
                f"  Avg user rating: {latest_metrics.avg_user_rating:.1f}/5.0"
            )
    ...
    def _analyze_single_model(self, model, days):
        ...
        # Basic metrics
        self.stdout.write(
            f'Training accuracy: {model.training_accuracy if model.training_accuracy is not None else "N/A"}'
        )
        self.stdout.write(
            f'Validation accuracy: {model.validation_accuracy if model.validation_accuracy is not None else "N/A"}'
        )

        # Recent performance
        cutoff_date = timezone.now() - timedelta(days=days)
        recent_feedback = FeedbackLearning.objects.filter(created_at__gte=cutoff_date)
