...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        deactivate_parser = subparsers.add_parser(
            "deactivate", help="Deactivate a model"
        )
        deactivate_parser.add_argument(
            "model_version", help="Model version to deactivate"
        )
    ...
    def handle_list(self, options):
        ...
        # Table header
        self.stdout.write(
            f'{"Version":<30} {"Type":<15} {"Active":<8} {"Accuracy":<10} {"Created":<20}'
        )
        ...
        for model in models:
            version = (
                model.version[:28] + "..." if len(model.version) > 30 else model.version
            )
            active = "✓" if model.is_active else "✗"
            accuracy = (
                f"{model.performance_metrics.get('validation_accuracy', 0):.3f}"
            ...
            self.stdout.write(
                f"{version:<30} {model.model_type:<15} {active:<8} {accuracy:<10} {created:<20}"
            )
    ...
    def handle_status(self, options):
        ...
        if active_models.exists():
            ...
            for model in active_models:
                ...
                self.stdout.write(
                    f"  {model.model_type}: {model.version} (accuracy: {accuracy})"
                )
    ...
    def handle_activate(self, options):
        ...
        with transaction.atomic():
            ...
            MLModel.objects.filter(model_type=model.model_type, is_active=True).update(
                is_active=False
            )
    ...
    def handle_delete(self, options):
        ...
        if model.is_active and not options["confirm"]:
            if not self._confirm_action(
                f"Model {model_version} is currently active. Delete anyway?"
            ):
        ...
        if not options["confirm"]:
            if not self._confirm_action(
                f"Delete model {model_version}? This cannot be undone."
            ):
                self.stdout.write("Deletion cancelled")
                return
    ...
    def handle_cleanup(self, options):
        ...
        for model_type in model_types:
            ...
            old_models = MLModel.objects.filter(
                model_type=model_type, is_active=False
            ...
            self.stdout.write(
                f"Cleaning up {model_type} models (keeping {keep_count} most recent)"
            )
