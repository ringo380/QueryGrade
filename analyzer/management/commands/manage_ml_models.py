...
class Command(BaseCommand):
    ...
    def handle_list(self, options):
        ...
        # Table header
        self.stdout.write(
            f'{"Version":<30} {"Type":<15} {"Status":<12} {"Accuracy":<10} {"Created":<20}'
        )
        ...
        for model in models:
            ...
            accuracy = (
                f"{model.validation_accuracy:.3f}"
            ...
            self.stdout.write(
                f"{version:<30} {model.model_type:<15} {model.status:<12} {accuracy:<10} {created:<20}"
            )
    ...
    def handle_activate(self, options):
        ...
        with transaction.atomic():
            ...
            MLModel.objects.filter(model_type=model.model_type, status="ACTIVE").update(
                status="DEPRECATED"
            )
                ...
                writer.writerow(
                    [
                        ...
                        (
                            ...
                            if model.training_accuracy is not None
                            else ""
                        ),
