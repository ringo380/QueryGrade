...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--source',
            type=str,
            help='Load from specific documentation source only'
        )
        ...
        parser.add_argument(
            '--export-rules',
            type=str,
            help='Export loaded rules to JSON file'
        )
    ...
    def _validate_benchmarks(self, loader, options):
        ...
        validation_results = loader.validate_benchmarks(sample_size=options['sample_size'])
        ...
        for benchmark in loader.benchmarks:
            if benchmark.validated:
                try:
                    ...
                    if options['verbose']:
                        self.stdout.write(
                            self.style.WARNING(f'Could not extract features for benchmark: {str(e)}')
                        )
    ...
    def _test_rule_application(self, loader, options):
        ...
        for sql_text in test_queries:
            ...
            self.stdout.write(f'  Score impact: {rule_results["total_score_impact"]:.1f}')
    ...
    def _download_external_documentation(self, loader, options):
        ...
        web_sources = [s for s in loader.sources if s.source_type == 'web' and s.enabled]
        ...
        for source in web_sources:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f'Failed to download from {source.name}: {str(e)}')
                )
