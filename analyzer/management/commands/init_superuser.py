...
class Command(BaseCommand):
    help = "Create a superuser from SUPERUSER_EMAIL / SUPERUSER_PASSWORD if none exists."

    def handle(self, *args, **options):
        User = get_user_model()
        email = os.environ.get("SUPERUSER_EMAIL", "").strip()
        password = os.environ.get("SUPERUSER_PASSWORD", "").strip()
        username = os.environ.get("SUPERUSER_USERNAME", "").strip() or email.split("@")[0] if email else ""

        if not email or not password:
            self.stdout.write("init_superuser: SUPERUSER_EMAIL/SUPERUSER_PASSWORD not set, skipping.")
            return
        ...
        self.stdout.write(self.style.SUCCESS(f"init_superuser: created {username} <{email}>"))
