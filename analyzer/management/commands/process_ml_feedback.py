...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--query-id',
            type=int,
            help='Process feedback for a specific query ID'
        )
    ...
    def show_feedback_statistics(self, options):
        ...
        self.stdout.write(f'Positive feedback: {positive_feedback} ({positive_feedback/total_feedback*100:.1f}%)')
        self.stdout.write(f'Negative feedback: {negative_feedback} ({negative_feedback/total_feedback*100:.1f}%)')

        # Recent feedback
        days = options['days']
        recent_cutoff = timezone.now() - timedelta(days=days)
        recent_feedback = QueryFeedback.objects.filter(created_at__gte=recent_cutoff).count()
        ...
        queries_with_feedback = Query.objects.filter(queryfeedback__isnull=False).distinct().count()
        total_queries = Query.objects.count()
        self.stdout.write(f'Queries with feedback: {queries_with_feedback}/{total_queries} ({queries_with_feedback/total_queries*100:.1f}%)')
    ...
    def _get_queries_to_process(self, options):
        ...
        for query in queryset:
            ...
            if feedback_count >= min_feedback:
                ...
                if not options['force_all']:
                    existing_training_data = TrainingData.objects.filter(query=query).exists()
    ...
    def _show_dry_run_info(self, queries_to_process, options):
        ...
        for query in queries_to_process[:10]:  # Show first 10
            ...
            self.stdout.write(f'Query {query.id} ({query.query_type}): {feedback_count} feedback items')
    ...
    def _process_feedback(self, queries_to_process, options):
        ...
        for i, query in enumerate(queries_to_process, 1):
            if verbose:
                self.stdout.write(f'Processing query {i}/{len(queries_to_process)}: {query.id}')

            try:
                # Collect feedback and create training data
                training_data = self.feedback_collector.collect_feedback_for_query(query.id)
                ...
                if training_data:
                    ...
                    if verbose:
                        self.stdout.write('  No training data created (insufficient feedback)')

            except Exception as e:
                if verbose:
                    self.stdout.write(
                        self.style.ERROR(f'  Error processing query {query.id}: {str(e)}')
                    )
                continue
    ...
    def _analyze_feedback_quality(self, options):
        ...
        user_stats = User.objects.filter(
            ...
            feedback_count__gte=5  # Users with at least 5 feedback items
        ...
        for min_complexity, max_complexity, label in complexity_ranges:
            ...
            if queries.exists():
                avg_agreement = QueryFeedback.objects.filter(
                    query__in=queries
                ).aggregate(avg=Avg('score_agreement'))['avg'] or 0

                self.stdout.write(f'  {label} ({min_complexity}-{max_complexity}): {avg_agreement:.1f} avg agreement')
    ...
    def _calculate_user_reliability(self, user):
        ...
        reliability = max(0, 1 - variance / 4)  # 4 is max possible variance for 1-5 scale
